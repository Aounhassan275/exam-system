...
<body class="bg-slate-800">
				...
				<form class="login-form" method="POST" action="{{route('register')}}" enctype="multipart/form-data">
					...
					<div class="card mb-0">
						<div class="card-body">
							...
							<div class="form-group form-group-feedback form-group-feedback-left college_fields" {{old('role_id')?old('role_id') == 2:'hidden'}}>
    ...
    <script>
        $(document).on('change', '#role_id', function () {
			...
			if(role_id == 2)
			{
				$('.college_fields').attr("hidden",false);
			}else{
				$('.college_fields').attr("hidden",true);
			}
        });
